...
void BMPToYUVConverter::BgrToY(const uint8_t* bgr, size_t bgrStride, size_t width, size_t height,
    ...
    const size_t numThreads = std::thread::hardware_concurrency(); // общее число потоков
    std::vector<std::thread> threads;

    size_t rowsPerThread = height / numThreads;
    for (size_t i = 0; i < numThreads; ++i) { // деления данных для каждого потока 
        size_t startRow = i * rowsPerThread; 
        size_t endRow = (i == numThreads - 1) ? height : (i + 1) * rowsPerThread;

        threads.emplace_back([=](...) { // копирования помогает каждому потоку работать со своими данными 
...
void BMPToYUVConverter::BgrToUV(const uint8_t* bgr, size_t bgrStride, size_t width, size_t height,
    ...
    size_t rowsPerThread = height / (2 * numThreads); // Учитываем шаг 2 для UV
    for (size_t i = 0; i < numThreads; ++i) {
        size_t startRow = i * rowsPerThread * 2;
        size_t endRow = (i == numThreads - 1) ? height : (i + 1) * rowsPerThread * 2;
